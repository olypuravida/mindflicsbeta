...
import type { SignUpFormAdminsValues } from './props-types'
...
import { AdminIdField } from './components/info/admin-id'
import { NameschoolField } from './components/nameschool'
import { createAdmin } from '@/infra/services/mindflics/admins'
import type { AdminAttributes } from '../../../../domain/db/mindflics/Admin/types'
...
export function SignUpFormAdmins() {
  ...
  const formCtx = useForm<SignUpFormAdminsValues>({
    defaultValues: {
      firstName: '',
      lastName: '',
      birthDate: '',
  ...
  const onSubmit = useCallback(async (data: SignUpFormAdminsValues) => {
    setError(null)
    setIsLoading(true)
    ...
    try {
      ...
      const dataAdmin:AdminAttributes = {
        data: {
      ...
      const { content: { message, user } } =  await createAdmin(dataAdmin)
      console.log(message)
